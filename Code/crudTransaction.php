...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id_histori = $_POST['id_histori'] ?? null;
    $action = $_POST['action'] ?? null;

    if ($action === 'delete' && $id_histori) {
        // HAPUS TRANSAKSI
        $sql = "DELETE FROM histori_transaksi WHERE id_histori = '$id_histori'";
    ...
    if ($action === 'edit' && $id_histori) {
        // TAMPILKAN FORM EDIT
        $sql = "SELECT * FROM histori_transaksi WHERE id_histori = '$id_histori'";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            // Form Edit
            echo "<form action='crudTransaction.php' method='POST'>";
            echo "<input type='hidden' name='id_histori' value='" . $data['id_histori'] . "' />";
